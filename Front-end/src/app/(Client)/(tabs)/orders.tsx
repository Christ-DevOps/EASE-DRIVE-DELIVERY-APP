...
const OrdersScreen = () => {
  ...
  // useRef so Animated.Value persists between renders
  ...
  // Keep filteredOrders in sync whenever orders or activeCategory change
  useEffect(() => {
    if (!orders || orders.length === 0) {
      setFilteredOrders([]);
      return;
    }
    const filtered = orders.filter(order => order.statusType === activeCategory);
  ...
  const handleCategoryPress = (category: string) => {
    ...
    // filteredOrders will update via effect above
  };
  ...
  const openModalWithOrder = (order: Order, track = false) => {
    setSelectedOrder(order);
    setTrackingMode(track);

    // show modal first (so content is mounted), reset animation and then animate
  ...
  const handleOrderPress = (order: Order) => {
    openModalWithOrder(order, false);
  };

  // when user presses track from a list (we receive order id)
  const handleTrackPress = (orderId: string) => {
    const order = orders.find(o => o.id === orderId);
    if (!order) {
      console.warn('Requested order for tracking not found:', orderId);
      // Optionally refetch single order or show an error toast
      return;
    }
    openModalWithOrder(order, true);
  };
...
const styles = StyleSheet.create({
  ...
  activeTab: { backgroundColor: '#FF762211', borderRadius: 12 },
  tabText: { fontSize: 16, fontWeight: '600', color: '#777' },
  activeTabText: { color: '#FF7622', fontWeight: '700' },
  activeIndicator: {
    position: 'absolute',
    bottom: 5,
    height: 3,
  ...
  listContainer: { paddingHorizontal: 20, paddingTop: 20, paddingBottom: 30, flexGrow: 1 },
  emptyContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 40, paddingTop: 50 },
  emptyImage: { width: 200, height: 200, marginBottom: 30, opacity: 0.7 },
  emptyTitle: { fontSize: 22, fontWeight: '700', marginBottom: 10, textAlign: 'center', color: '#333' },
  emptyText: { fontSize: 16, color: '#777', textAlign: 'center', marginBottom: 30, lineHeight: 24, maxWidth: 300 },
  loadingContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', paddingBottom: 100 },
  loadingText: { marginTop: 20, fontSize: 16, color: '#FF7622' },
